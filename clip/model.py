...
class CLIP(nn.Module):
    ...
    def initialize_parameters(self):
        ...
        for transformer in [self.input_transformer, self.motion_transformer]:
            proj_std = (transformer.width ** -0.5) * ((2 * transformer.layers) ** -0.5)
            attn_std = transformer.width ** -0.5
        ...
        if self.input_out_proj is not None:
            nn.init.normal_(self.input_out_proj, std=self.input_transformer.width ** -0.5)
            
        if self.motion_out_proj is not None:
            nn.init.normal_(self.motion_out_proj, std=self.motion_transformer.width ** -0.5)
    ...
    def encode_motion(self, motion):
        assert motion.shape[1] == self.context_length, f"Length is {motion.shape[1]}, but should be {self.context_length}, padd to {self.context_length} or chop and do window run, this is to prevent messing up with positional embeddings"

        motion = self.motion_in_proj(motion)
    ...
    def encode_inputs(self, inputs):
        assert inputs.shape[1] == self.context_length, f"Length is {inputs.shape[1]}, but should be {self.context_length}, padd to {self.context_length} or chop and do window run, this is to prevent messing up with positional embeddings" 
                
        inputs = self.input_in_proj(inputs)

        pos_seq = torch.arange(inputs.shape[1], device=inputs.device).to(inputs.dtype)
        pos_emb = self.pos_emb(pos_seq) 
        ...
        # x = x[torch.arange(x.shape[0]), inputs.argmax(dim=-1)] @ self.input_out_proj
    ...
    def forward(self, inputs, motion):
        ...
        input_features = self.encode_inputs(inputs).mean(dim=1)
